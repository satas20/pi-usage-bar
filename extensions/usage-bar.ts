 ...
 *
 * Renders a compact usage strip in a widget row below the editor:
 *
...
async function piAuth(id: string): Promise<PiAuthEntry | null> {
  try {
    const auth = JSON.parse(await readFile(join(AGENT_DIR, "auth.json"), "utf8")) as Record<
      string,
      PiAuthEntry
    >
    return auth[id] ?? null
...
function asTable(v: unknown): JsonTable {
  return v && typeof v === "object" && !Array.isArray(v) ? (v as JsonTable) : {}
}
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    const render = () => {
      ...
      for (const p of enabled) {
        ...
        const windows = (byProvider[p.id] ?? []).filter((w) => w.resetsAt > now)
        if (windows.length > 0)
          groups.push({ short: p.short, status: byStatus[p.id] ?? "none", windows })
      }
      ...
      const statusColor = (s: StatusIndicator) =>
        s === "critical" || s === "major" ? ("error" as const)
        : s === "minor" ? ("warning" as const)
        : ("accent" as const)
      ...
      for (const g of groups) {
        ...
        for (const w of g.windows) {
          ...
          if (config.showBars) {
            const filled = Math.min(barWidth, Math.max(0, Math.round((pctOf(w) / 100) * barWidth)))
            win.push(
              theme.fg(colorOf(w), "▓".repeat(filled)) + theme.fg("dim", "░".repeat(barWidth - filled)),
            )
          }
    ...
    // Live countdown; setWidget is only called when the rendered line changes
